...
public sealed class UncheckCheckableHandlerTests
{
    ...
    {
        var act = () => new UncheckCheckableHandler(null!, _messenger);
    ...
    {
        var act = () => new UncheckCheckableHandler(_dataService, null!);
    ...
    public async Task HandleAsync_Sends_CommandFailed_When_UpsertAsync_Returns_Error()
    {
        ...
        var checkable = new Checkable(true, Guid.NewGuid(), DateTimeOffset.UtcNow, "Task", checkableId, Guid.NewGuid());
        var command = new UncheckCheckable(checkableId);
    ...
    [Fact]
    public async Task HandleAsync_Sets_Checked_To_False_Before_Upserting()
    {
        var checkableId = Guid.NewGuid();
        var checkable = new Checkable(true, Guid.NewGuid(), DateTimeOffset.UtcNow, "Task", checkableId, Guid.NewGuid());
        var command = new UncheckCheckable(checkableId);
    ...
    [Fact]
    public async Task HandleAsync_Sends_CheckableUnchecked_When_Command_Succeeds()
    {
        var checkableId = Guid.NewGuid();
        var checkable = new Checkable(true, Guid.NewGuid(), DateTimeOffset.UtcNow, "Task", checkableId, Guid.NewGuid());
        var command = new UncheckCheckable(checkableId);
        ...
        var sentEvent = _messenger.ReceivedCalls().Should().ContainSingle().Subject.GetArguments()[0];
        sentEvent.Should().BeOfType<CheckableUnchecked>().Which.CommandId.Should().Be(command.Id);
    }
