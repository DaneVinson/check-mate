...
- Serves the CM.Quasar SPA: `UseDefaultFiles()` and `UseStaticFiles()` serve from `wwwroot/`; `MapFallbackToFile("index.html")` enables Vue Router history mode
- `CM.Api.csproj` has two MSBuild targets for building the SPA:
  - `BuildQuasarIfMissing` (`BeforeTargets="Build"`, condition `!Exists('$(MSBuildThisFileDirectory)wwwroot\index.html')`) — runs `npm install` + `quasar build` + copies `dist/spa/` → `wwwroot/` only when missing (fast incremental builds)
...
Applications/CM.Quasar/
  ...
  quasar.config.ts     — SPA mode, history router, dark theme, SCSS, dev proxy, TypeScript strict
  tsconfig.json        — extends .quasar/tsconfig.json (auto-generated by quasar prepare)
  src/
    App.vue            — root Vue component
    boot/
      axios.ts         — placeholder; future axios instance + JWT Bearer interceptor
      signalr.ts       — placeholder; future @microsoft/signalr HubConnection to /events
    css/
      app.scss         — global stylesheet
      quasar.variables.scss — Quasar SCSS color variable overrides
    layouts/
      MainLayout.vue   — QLayout with nav drawer; toolbar showing "Check Mate"; nav link to /checklists
    pages/
      CheckListDetailPage.vue — QList of CheckableDto[] with checkboxes; loads via stores (TODO)
      CheckListsPage.vue      — QList of CheckListDto[]; loads via checkLists store (TODO)
      ErrorNotFound.vue       — 404 page
      LoginPage.vue           — email + name sign-in form; calls auth store login (TODO)
    router/
      index.ts         — creates Vue Router in history mode
      routes.ts        — /login → LoginPage; / with MainLayout → /checklists, /checklists/:id; catchAll → ErrorNotFound
    stores/
      auth.ts          — Pinia store; accessToken, expiresAt, isAuthenticated getter; login/logout actions (TODO: call API)
      checkables.ts    — Pinia store; checkables: CheckableDto[]; loadByCheckList, check, uncheck actions (TODO)
      checkLists.ts    — Pinia store; checkLists: CheckListDto[]; load(_userId) action (TODO)
    types/
...
- **TypeScript**: `{ strict: true, vueShim: true }`; `tsconfig.json` extends `.quasar/tsconfig.json` which is auto-generated by `quasar prepare` (run automatically on `npm install` via `postinstall` script)
- **Stores**: all three Pinia stores have placeholder `TODO` actions — not yet wired to the API
- **Boot files**: `axios.ts` and `signalr.ts` are commented-out placeholders — not registered in `quasar.config.ts` boot array yet
- **Building for production**: `dotnet build` on CM.Api triggers `BuildQuasarIfMissing` (first run only); `dotnet publish` always rebuilds; output lands in `Applications/CM.Api/wwwroot/`
- **Dev workflow**: run `npx quasar dev` from `Applications/CM.Quasar/` for hot-reload; proxy forwards API calls to `dotnet run` on port 5000
- TypeScript types in `src/types/` mirror the C# DTO positional record shapes (camelCase, all strings/booleans)
