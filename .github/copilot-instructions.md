...
| `CM.Quasar` | Quasar/Vue 3 SPA, TypeScript | Single-page application front-end served by CM.Api |
...
- `Applications/CM.Quasar` — Quasar SPA front-end
...
Applications/CM.Api/
  ...
  Program.cs           — top-level statements; registers JsonSerializerOptions and JwtOptions singletons; calls AddDefaultHandlers(), AddBogusDataServices(), AddSignalRMessenger(), AddFastEndpoints(); configures JWT Bearer auth and authorization middleware; maps CheckMateHub to /events; serves CM.Quasar SPA via MapGet("/quasar/{**path}") using FileExtensionContentTypeProvider
```
...
- Serves the CM.Quasar SPA via `app.MapGet("/quasar/{**path}", ...)` endpoint using `FileExtensionContentTypeProvider` and `ctx.Response.SendFileAsync`; files live in `wwwroot/quasar/`; non-existent paths fall back to `index.html` for Vue Router history mode; `_GlobalUsings.cs` includes `Microsoft.AspNetCore.StaticFiles`
- `CM.Api.csproj` has two MSBuild targets for building the SPA:
  - `BuildQuasarIfMissing` (`BeforeTargets="Build"`, condition `!Exists('$(MSBuildThisFileDirectory)wwwroot\quasar\index.html')`) — runs `npm install` + `quasar build` + copies `dist/spa/` → `wwwroot/quasar/` only when missing (fast incremental builds)
  - `BuildQuasar` (`BeforeTargets="Publish"`) — always rebuilds fresh; uses `<ResolvedFileToPublish>` to include the SPA output
- `SpaRoot` MSBuild property points to `..\.\CM.Quasar\`
- `Applications/CM.Api/wwwroot/` is git-ignored (generated output)
...
```
Applications/CM.Quasar/
  .editorconfig
  .gitignore           — ignores node_modules/, dist/, .quasar/
  index.html           — SPA entry point; contains <!-- quasar:entry-point -->
  package.json         — name: cm-quasar; scripts: dev (quasar dev), build (quasar build), postinstall (quasar prepare)
  quasar.config.ts     — SPA mode, publicPath: '/quasar/', history router, dark theme, SCSS, dev proxy, TypeScript strict
  tsconfig.json        — extends .quasar/tsconfig.json (auto-generated by quasar prepare)
  src/
    App.vue            — root Vue component; persistent q-dialog auth modal (cannot dismiss); Name field then Email field; Login (far left) and Create Account (far right) buttons; calls auth.initialize() on startup
    api/
      index.ts         — postCommand(type, payload), postQuery<T>(type, payload), postAuth(email, name); apiFetch() attaches Bearer token from auth store
    boot/
      axios.ts         — placeholder (not registered)
      signalr.ts       — placeholder (not registered)
    css/
      app.scss         — global stylesheet
      quasar.variables.scss — Quasar SCSS color variable overrides
    layouts/
      MainLayout.vue   — QLayout; toolbar showing "Check Mate" + logout button (navigates to /checklists and calls auth.logout())
    pages/
      CheckListDetailPage.vue — full checkable list detail view (see below)
      CheckListsPage.vue      — full check lists view (see below)
      ErrorNotFound.vue       — 404 page
    router/
      index.ts         — creates Vue Router in history mode
      routes.ts        — / with MainLayout → /checklists, /checklists/:id; catchAll → ErrorNotFound
    stores/
      auth.ts          — Pinia store; accessToken, expiresAt, userId, userName state; isAuthenticated getter; initialize() restores from localStorage; login(), createAccount(), logout() (resets checkLists + checkables stores); _storeToken() decodes JWT payload for userId/userName
      checkables.ts    — Pinia store; checkables: CheckableDto[]; loadByCheckList, check, uncheck, create, updateDescription, deleteCheckable actions
      checkLists.ts    — Pinia store; checkLists: CheckListDto[]; load, create, updateName, deleteCheckList actions
      index.ts         — exports createPinia() (Quasar auto-detects for Pinia registration)
    types/
      CheckableDto.ts  — { id, checked: boolean, checkListId, created, description, userId: string }
      CheckListDto.ts  — { id, created, name, userId: string }
      UserDto.ts       — { id, created, email, name: string }
```

### CM.Quasar Notes
- **Stack**: Quasar 2.x / @quasar/app-vite, Vue 3, TypeScript (strict), Vite, Pinia, Vue Router, SCSS/sass-embedded
- **Theme**: dark mode enabled globally via `config: { dark: true }` in `quasar.config.ts`
- **SPA route**: served at `/quasar/` — `quasar.config.ts` sets `publicPath: '/quasar/'`; CM.Api uses a `MapGet("/quasar/{**path}")` endpoint with `FileExtensionContentTypeProvider` serving from `wwwroot/quasar/`; falls back to `index.html` for Vue Router history mode
- **Dev server proxy**: `/auth`, `/commands`, `/queries` → `http://localhost:5000`; `/events` → WebSocket proxy (`ws: true, changeOrigin: true`)
- **Extras**: `roboto-font`, `material-icons`
- **TypeScript**: `{ strict: true, vueShim: true }`; `tsconfig.json` extends `.quasar/tsconfig.json`
- **Pinia**: registered via `src/stores/index.ts` exporting `createPinia()` — Quasar auto-detects this file
- **Boot files**: `axios.ts` and `signalr.ts` are placeholders — not registered in `quasar.config.ts` boot array
- **Build workflow**: `Set-Location CM.Quasar; node_modules\.bin\quasar build` then copy `dist\spa` → `CM.Api\wwwroot\quasar`; use `node_modules\.bin\quasar` not `npx quasar`
- **MSBuild targets**: `BuildQuasarIfMissing` checks for `wwwroot\quasar\index.html`; `BuildQuasar` always rebuilds on publish
- **TypeScript types** in `src/types/` mirror the C# DTO positional record shapes (camelCase)

### CheckListsPage.vue
- Header: possessive user name (`"Bilbo's Lists"` or `"Moss' Lists"` for s-ending names) + `+` add button
- Items: `q-banner` with `bg-grey-9`, `dense rounded inline-actions`, `cursor-pointer` (unless editing)
- Clicking banner body navigates to `/checklists/:id`
- Edit button (pencil icon): switches banner body to inline `q-input`; blur cancels; save button sends `UpdateCheckList`
- New item: `+` prepends an empty banner in edit mode (same inline pattern); blur removes if empty; save sends `CreateCheckList`; no dialog
- Uses local `editableLists: ref<EditableCheckList[]>` (not a computed from store) with `editing`, `isNew`, `editName` fields
- Watches `auth.isAuthenticated`: loads lists when it becomes true (after login), clears when it becomes false (after logout)

### CheckListDetailPage.vue
- Header: back button → `/checklists`; list name + small delete icon button (`dense`, `size="sm"`, `q-ml-xs`) + `+` add button
- Delete list: clicking delete icon opens a `q-dialog` modal with "Cancel" and "Confirm Delete" (negative); sends `DeleteCheckList` then navigates back
- **Unchecked items** (`q-banner bg-grey-9`):
  - `#avatar`: round flat dense secondary `check_circle` button (hidden while editing); clicking sends `CheckCheckable` and moves item to checked list
  - Body: description text or inline `q-input` when editing; blur cancels (removes if new)
  - `#action`: edit icon → save icon (disabled if empty); `@mousedown.prevent` prevents blur race; new items use `CreateCheckable` on save, existing use `UpdateCheckable`
  - Sorted alphabetically; new items sort first
- **Checked items** (collapsible `q-expansion-item`, `Checked (N)`, `text-grey-6`):
  - `q-banner bg-grey-9` (turns `bg-negative` when `pendingDelete = true`); clicking banner body resets `pendingDelete = false`
  - `#avatar`: round flat dense grey-6 `check_circle` button (disabled when `pendingDelete`); clicking sends `UncheckCheckable` and moves item back
  - Body: plain grey-6 text (no strikethrough, not editable)
  - `#action`: delete icon → sets `pendingDelete = true`; when pending: white outline "Confirm delete" button (`@click.stop`); clicking sends `DeleteCheckable` and removes item
  - Sorted alphabetically
- Uses local `editableItems: ref<EditableCheckable[]>` with `editing`, `isNew`, `editDescription`, `pendingDelete` fields
